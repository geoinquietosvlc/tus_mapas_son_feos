=======================================================================
Vuestros mapas son feos y no sabéis por qué
=======================================================================

Introducción
============

*Caveat Emptor*::

    Este es un taller **especulativo,** no científico.

    No hay una receta para hacer las cosas bien, no hay una bala de plata.


Habría que tener claros unos conceptos

* Diseño cartográfico >> Arte cartográfico >> Estética cartográfica

* Útil / no útil **¿Comuníca?**

* Feo / Bonito **¿Es estético?**

Hay dos componentes diferenciadas:

* El objetivo (Mensaje / Qué)

* El lenguaje (Diseño / Cómo)

75% de las veces (estadísitca inventada) no sabemos ni el qué ni el cómo de un
mapa... y estos acaban siendo feos.

Existe una alta probabilidad de que al final, tus mapas sigan siendo feos...
espero que por lo menos sepas por qué.

Contenidos
==========

Antes de ver de qué vamos a hablar... de qué **NO** vamos a hablar (pero
deberías mirártelo):

**Tabla de NO Contenidos:**

* Teoría de la comunicación.
* Reproducción cartográfica.
* Cartografía matemática:

  + Escalas
  + Coordenadas
  + Proyecciones (La tierra no es plana pero tu mapa sí)

* La percepción
* Simbología
* Geografía
* Herramientas cartográficas

  + Software
  + Hardware

.. only:: singlehtml

   De lo que **SÍ** vamos a hablar es:

   * Parte Técnica

     + Variables visuales
     + Composicion

   * Parte Emocional
   * Referencias
    
   **Tabla de Contenidos:**


.. toctree::
   :maxdepth: 2
   :caption: Tabla de Contenidos:

   010/010010_tecnica.rst
   020/020010_emocion.rst
   030/030010_notas_finales.rst
   900/900010_referencias.rst

